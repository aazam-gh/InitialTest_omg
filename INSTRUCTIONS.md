...
`npm run dev` starts the Express API and `npm start` starts the React development server. The API writes its selected port to `.server-port`, which the frontend proxy uses when the preferred API port is unavailable. See [SETUP.md](SETUP.md) for the repeatable walkthrough and [README.md](README.md) for the complete API and configuration reference.
...
- Accurate setup, environment, API, persistence, and contract-deployment guidance that matches the repository scripts
...
The current implementation addresses all three assessment tracks through a single dependency-injected backend, a browser-owned signing flow, an atomic validated persistence layer, and an independently deployable OpenZeppelin ERC-20 artifact.

Notable design decisions:

- HTTP endpoints are split across route and controller modules; ledger operations and persistence coordination live in services, while validation invariants live in the model.
- Wallet private keys are generated, encrypted, imported, and used only in the browser. The wallet API returns public account metadata rather than creating server-custodied keys.
- Pending outgoing transfers reserve spendable balance. A transfer becomes confirmed only after mining includes it in a valid proof-of-work block.
- Persisted state is restored into model instances and fully validated. Invalid state is retained under a diagnostic backup name and prevents startup.
- `AssessmentToken` uses standard OpenZeppelin ERC-20 balances, allowances, `Transfer`, and `Approval` behavior and remains intentionally separate from the educational JavaScript ledger. `npm run contract:deploy` uses an ephemeral Hardhat network; an external deployment requires `RPC_URL` and `DEPLOYER_PRIVATE_KEY` followed by `npx hardhat run scripts/deploy-contract.js --network external`.

The verification commands for this submission are listed in [README.md](README.md#verification), and its deliberate limitations are documented in [README.md](README.md#deliberate-boundaries).
